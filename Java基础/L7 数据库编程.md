...
7. （可选步骤）[Maven本地配置](#^467131)

**第四步：在 IDEA 中配置 Maven**

8. 打开 Java 项目，进入 IDEA 设置顶部菜单栏搜索 `Maven`；

>[!tip] 配置信息
>- `Maven home directory：D:\ProgramEnviron\Java\jdbc\apache-maven-3.9.12`
  (选择 Maven 解压根目录,不要选择默认的自带 Maven)
>- `User settings file：D:\ProgramEnviron\Java\jdbc\apache-maven-3.9.12\conf\settings.xml`
  (选择修改后的 settings.xml 文件)
>- Local repository：会自动读取settings.xml中的配置，无需手动修改

9. 点击「Apply」→「OK」保存配置。

**第五步：将 JDBC 项目转为 Maven 项目并添加 MySQL 驱动依赖**

10. 普通 Java 项目转为 Maven 项目。（IDEA）右键项目→「Add Framework Support...」；
11. 在弹出的窗口中，勾选「Maven」，点击「OK」；
12. 项目会自动生成 `pom.xml` 文件（Maven 的核心配置文件），若未自动生成，可手动创建（放在项目根目录）。

13. 在 `pom.xml` 中添加 MySQL 驱动依赖

14. 打开项目根目录下的 `pom.xml` 文件；
15. 在 `<dependencies>` 标签内（若没有则手动添加 `<dependencies></dependencies>`），插入 MySQL 驱动依赖：
...
16. 保存 `pom.xml` 后，IDEA 会自动提示「Load Maven Changes」，点击加载即可；若未自动加载，右键 `pom.xml`→「Maven」→「Reload project」，Maven 会自动下载 MySQL 驱动到本地仓库。
